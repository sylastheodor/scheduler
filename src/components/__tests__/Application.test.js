...
describe("Application", () => {
  ...
  it("loads data, cancels an interview, and increases the spots remaining for the first day by 1", async () => {
    ...
    await waitForElement(() => getByText(container, 'Archie Cohen'));
    const appointments = getAllByTestId(container, "showbox");
    const appointment = appointments[0]
    fireEvent.click(getByAltText(appointment, "Delete"))
    await waitForElement(() => getByText(container, 'Confirm'))
    fireEvent.click(getByText(container, 'Confirm'))
    await waitForElement(() => getByText(container, /deleting.../i))
    const day = getAllByTestId(container, "day").find((day) =>
      queryByText(day, "Monday")
    );
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    waitForElement(() => getByPlaceholderText(container, "Archie Cohen"));
    fireEvent.change(getByPlaceholderText(container, /Enter Student Name/i), {
      target: { value: "Minkus Fonda" },
    });
    fireEvent.click(getByAltText(container, "Sylvia Palmer"));
    fireEvent.click(getByText(container, "Save"));
    await waitForElement(() => getByText(container, "Minkus Fonda"));
    const day = getAllByTestId(container, "day").find((day) =>
    queryByText(day, "Monday")
  );
